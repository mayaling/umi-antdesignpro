...
export async function getInitialState(): Promise<{
  ...
  if (history.location.pathname !== loginPath) {
    ...
    const menuList = handleTree(menuData)
    return {
      fetchUserInfo,
      menuList,
      currentUser,
...
export const layout: RunTimeLayoutConfig = ({ initialState }) => {
  ...
  // const {routes} = _.isEmpty(initialState?.menuList)?{routes:[]}:initialState?.menuList
  return {
    // menuDataRender:()=>routes,
    menuDataRender: (menuList) => initialState.menuList || menuList, //这样写图标展示不出来
